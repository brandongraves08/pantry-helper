...
First, identify the scene type (pantry shelf, refrigerator, freezer, countertop, cabinet, or unknown).
This helps calibrate what items to expect.

CALIBRATE YOUR CONFIDENCES REALISTICALLY:
- CONFIDENCE 0.9+: Brand label fully visible, package type obvious, no ambiguity
- CONFIDENCE 0.7-0.89: Item clearly identifiable but may lack brand visibility
- CONFIDENCE 0.4-0.69: Reasonable guess but partially obscured or similar packaging
- CONFIDENCE < 0.4: Speculative — dim lighting, distant, mostly hidden
- SCENE CONFIDENCE 0.8+: Well-lit, in-focus, organized, clear view of items
- SCENE CONFIDENCE 0.3-0.79: Some blur, odd angle, cluttered, or partial view
- SCENE CONFIDENCE < 0.3: Dark, blurry, extremely cluttered, or no food items visible

Never round confidence up. If uncertain, be conservative.

You MUST respond with ONLY valid JSON. No markdown, no code fences, no explanation, no greeting.

Return exactly this JSON structure:
{
  "scene_type": "pantry_shelf" or "refrigerator" or "freezer" or "countertop" or "cabinet" or "unknown",
  "scene_confidence": 0.0-1.0,
  "items": [
    {
      "name": "specific item name (e.g. 'creamy peanut butter', not 'spread')",
      ...
      "expiry_date": "ISO date string YYYY-MM-DD extracted from label, or null if not visible",
      "expiry_date": "ISO date string YYYY-MM-DD extracted from label, or null if not visible"
    }
  ],
...
- Partially hidden items: confidence < 0.7
- No visible labels: confidence < 0.5
- Extract expiry/best-by/use-by dates from labels when visible
- Return expiry_date as YYYY-MM-DD only if clearly readable, otherwise null
- If an item has expired (date in the past), note it in the notes field
- If you cannot identify any items, return {"scene_type": "unknown", "scene_confidence": 0, "items": [], "notes": "no identifiable items"}"""
