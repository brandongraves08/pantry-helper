...
- **Simulator E2E PASSED** — "open pantry helper" → welcome; "add apples to the pantry list" → "Added apples..." AND row landed in shopping list (`apples x1 | voice`); "add 3 cans of black beans" → "Added 3 black beans..." (normalized); **one-shot "ask pantry helper to add beans to the shopping list" → "Added beans..." (verified, no open needed)**. Test stage enabled (the "Test is disabled" toggle is a react-select — click `.Select-control`, pick **Development**).
